        ...
        protected virtual void Initialize()
        {
            ...
            try
            {
                ...
                if (_navigator != null)
                {
                    ...
                    foreach (KeyValuePair<string, string> pair in _navigator.GetNamespacesInScope(XmlNamespaceScope.All))
                    {
                        if (!_xmlNamespaceManager.HasNamespace(pair.Key))
                            _xmlNamespaceManager.AddNamespace(pair.Key, pair.Value);
                    }
            ...
            try
            {
                ...
                foreach (XPathNavigator navigator in iterator)
                {
                    if (_xmlNamespaceManager == null)
                        _xmlNamespaceManager = new System.Xml.XmlNamespaceManager(navigator.NameTable);
                    ...
                    do
                    {
                        if (!string.IsNullOrEmpty(attributeNavigator.NamespaceURI))
                        {
                            string prefix = attributeNavigator.Prefix;
                            string uri = attributeNavigator.NamespaceURI;

                            if (!string.IsNullOrEmpty(uri))
                                if (!_xmlNamespaceManager.HasNamespace(prefix))
